...
# kawris_hash
RUN git clone https://github.com/kawrispay/kawris_hash
RUN cd kawris_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} kawris
RUN useradd -u ${USER_ID} -g kawris -s /bin/bash -m -d /kawris kawris
...
RUN mkdir /kawris-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /kawris-src && \
