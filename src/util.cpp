...
const char * const BITCOIN_CONF_FILENAME = "kawris.conf";
const char * const BITCOIN_PID_FILENAME = "kawrisd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "kawris" is a composite category enabling all Kawris-related debug output
                if(ptrCategory->count(std::string("kawris"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Kawris
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Kawris
    // Mac: ~/Library/Application Support/Kawris
    // Unix: ~/.kawris
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kawris";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kawris";
#else
    // Unix
    return pathRet / ".kawris";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty kawris.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override kawris.conf
